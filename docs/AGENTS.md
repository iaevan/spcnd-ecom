...
> five analysis reports at the repo root — this spec references them by file and section:
> - `woocommerce-comprehensive_report.md` — combined deep-dive
> - `woocommerce-core-architecture-report.md` — internals
> - `woocommerce-feature-parity-report.md` — feature checklist
> - `woocommerce-analysis.md` — earlier exploration
> - `woocommerce_api_reference.md` — REST reference
...
├── docs/                      # This file + AGENTS.md (the original archived spec) + the 5 WC analysis reports at repo root
├── turbo.json
├── pnpm-workspace.yaml
├── package.json
└── README.md
...
- **coupons** — full shape, see `woocommerce-comprehensive_report.md` §6 + `woocommerce-feature-parity-report.md` §10. Types: `fixed_cart`, `percent`, `fixed_product` (CHECK-constrained, plus a future `custom` allowing plugin-defined types via `compat-wc` event hook).
...
The full 45+ controller list is in the original AGENTS.md §14.3 and `woocommerce_api_reference.md`. All are implemented for both v1 and v3 — v3 controller is a thin serializer adapter over v1.
...
Product, Cart, Checkout, Order, Shipping, Tax, Coupon, Payment Gateway, Webhook, Lifecycle — all ~150 names from old §28.1 through §28.10. Reproduce verbatim. Behavioral triggers and signatures are in old §28 and in `woocommerce-core-architecture-report.md` §6 (cart), §7 (checkout), §8 (order) etc.
...
`compat-wc` is documented end-to-end in its package README and exercised by a port-test suite (`tests/compat/*.test.ts`) that takes real WC plugin behaviors from `woocommerce-analysis.md` and verifies the compat layer reproduces them.
...
Bus emits cache-invalidation events keyed by entity + version. WC's `DONOTCACHEPAGE`/`DONOTCACHEOBJECT`/`DONOTCACHEDB` semantics (per `woocommerce-analysis.md` §8) are reproduced as scoped invalidation scopes.
...
Full country/state data (250+ countries, all states, continents, phone codes, EU + VAT lists — see `woocommerce_comprehensive_report.md` §20.2). 150+ currencies with symbols. 40+ country address formats and 80+ country-local overrides. Reproduces all WC address-format and postcode-format rules (old §20.3, §20.6).
...
Reproduce exactly from old §27 and `woocommerce-core-architecture-report.md`:
...
- `woocommerce-comprehensive-report.md` — WC internals deep-dive.
- `woocommerce-core-architecture-report.md` — WC architecture deep-dive.
- `woocommerce-feature-parity-report.md` — feature parity checklist.
- `woocommerce-analysis.md` — earlier analysis (data tables, controllers).
- `woocommerce_api_reference.md` — REST controller reference.
